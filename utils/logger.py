...
import numpy as np
from utils.utils import generate_bmode, norm_data
...
class LoggerWriter:
    ...
    def set_predict_save(self):
        ...
        self.bmode_save_root = os.path.join(self.log_dir, 'bmode_save')
        if os.path.exists(self.bmode_save_root):
            # return
            rmtree(self.bmode_save_root)
        os.mkdir(self.bmode_save_root)
            
    def write_predict(self, predictions, names):
        # prediction is a tensor with shape (batch, h, w)
        # names is list with length batch        
        for y, name in zip(predictions, names):
            save_file_path = os.path.join(self.predict_save_root, name)
            np.save(save_file_path, y)
            
    def write_predict_image(self, predictions, names):       
        for y, name in zip(predictions, names):
            save_file_path = os.path.join(self.bmode_save_root, name.replace('.npy', '.png'))
            bmode = generate_bmode(y+EPSLION)
            bmode2img = Image.fromarray(norm_data(bmode, [0, 255])).convert("L")
            bmode2img.save(save_file_path, format='png')
